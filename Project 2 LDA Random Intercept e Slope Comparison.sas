...
/* Random Slope Complete + Time_quad*/
proc glimmix data=alzheimer_long_centered method=quad(qpoints = 30);
	class PATID SEX;
	model CDRSB_CAT(event = '1') = TIME AGE_STD BMI_STD SEX ADL ABPET_STD TAUPET_STD
	TIME_BMI TIME_TAU TIME_SEX TIME_AGE TIME_AB TIME_ADL
	TIME_QUAD TIME_SEX_QUAD TIME_AGE_QUAD TIME_BMI_QUAD TIME_ADL_QUAD TIME_AB_QUAD TIME_TAU_QUAD
...
/* Higher BIC --> keep it linear */


/* Random Intercept Complete*/
proc glimmix data=alzheimer_long_centered method=quad(qpoints = 30);
	class PATID SEX;
	model CDRSB_CAT(event = '1') = TIME AGE_STD BMI_STD SEX ADL ABPET_STD TAUPET_STD
	TIME_BMI TIME_TAU TIME_SEX TIME_AGE TIME_AB TIME_ADL
...
proc glimmix data=alzheimer_long_centered method=quad(qpoints = 30);
	...
	/* ATTENZIONE: Il dataset di output è 'predictions_data' */
	output out=predictions_data
           pred(ilink)=P_FIXED       /* Predizione della media di Popolazione */
           predicted(ilink)=P_BLUP; /* Predizione Specifica per Soggetto (Non necessaria per questo plot) */
run;
